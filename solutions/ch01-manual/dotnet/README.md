...
with the .NET/SQL Server baseline and want the complete executable form of a step, you
...
run on the existing .NET legacy source VM from Challenge 0 so the frozen topology guard
can verify the host.
...
an authenticated isolated facilitator profile. It must also already contain the three
protected parameter files this slice deploys with, under `C:\protected\`:

| File | What it selects |
| --- | --- |
| `manual-dotnet-bootstrap.json` | `deploymentStage=bootstrap`, `applicationRevisionRole=""` |
| `manual-dotnet-baseline.json` | `deploymentStage=application`, `applicationRevisionRole=baseline` |
| `manual-dotnet-release.json` | `deploymentStage=application`, `applicationRevisionRole=release` |
...
and this slice's fixed `stack=dotnet-sqlserver`/`imageProvider=azure-files` selection.
Each is a standard ARM parameter document. They are inputs; never copy one into the
repository.
...
commit the handoff records, and builds `dotnet/Dockerfile` from that checkout. A commit
that exists only on this VM satisfies neither, so publish before you record any identity.
Author `dotnet/Dockerfile` first — step 5 states exactly what it must contain — then run
this from `C:\MicroHack\source`:

```powershell
git add --all
git commit -m 'Complete manual modernization slice manual-dotnet'
...
$ResourceGroup = (Get-Content 'C:\protected\manual-dotnet-bootstrap.json' -Raw |
...
$Smoke = Get-Content 'C:\MicroHack\status\dotnet-smoke.json' -Raw | ConvertFrom-Json
if ($Smoke.stack -cne 'dotnet' -or $Smoke.figures -ne 198 -or $Smoke.categories -ne 20) {
...
Build and review the shared target IaC. The protected files are inputs; never copy them
into the repository.
...
The `dotnet/Dockerfile` you author uses digest-pinned build/runtime images and numeric
...
`sourceCommit=$SourceCommit` and `imageDigest=$ImageDigest`. Run what-if for each and
verify the image is
...
**Other stack:** [Manual Java](../java/README.md) ·
