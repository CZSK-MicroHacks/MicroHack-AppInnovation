...
- Refroze the toolchain with repository-verified Azure SDK packages, SqlPackage `170.4.83`, and exact linux/amd64 manifest digests for .NET SDK 8.0.424, ASP.NET 8.0.30, and Microsoft OpenJDK 17.0.20+8. The Java JDBC path uses stable `azure-identity-extensions` rather than the older preview PostgreSQL provider.
