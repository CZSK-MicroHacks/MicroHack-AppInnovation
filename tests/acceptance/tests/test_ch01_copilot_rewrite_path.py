...
def test_rewrite_migration_is_powershell_native_on_windows_source_vm(
    ...
    """Both native migration guides execute from the exact Windows source VM."""
    for content in _rewrite_solution_documents(repo_root).values():
        ...
        # The export writes this artifact, so it must land somewhere the participant can
        # write. C:\protected is facilitator-supplied and granted to them read-only.
        assert "c:\\programdata\\microhack\\migration\\" in migration
        assert "$databaseartifact = 'c:\\protected" not in migration
        # Identity still has to be proven before the bootstrap target output is consumed.
        assert migration.index("git rev-parse head") < migration.index(
            "test-path -literalpath $targetoutput"
        )
        assert migration.index("test-path -literalpath $targetoutput") < migration.index(
        ...
        for case, native_exit, acceptance_exit, should_commit in (
            ...
            environment = {
                **os.environ,
                ...
                # The block names the slice through a required variable, so supply the
                # value a participant would set rather than letting its guard fire.
...
def test_rewrite_cutover_publishes_digest_and_captures_ordered_release(
    ...
    for stack, content in _rewrite_solution_documents(repo_root).items():
        ...
        # The provisioned VM has no Docker daemon, so the image is built server-side
        # inside the registry created by bootstrap, and only after it exists.
        for daemon_command in ("docker build", "docker buildx", "docker push",
                               "docker tag", "docker image inspect"):
            assert daemon_command not in normalized
        build_invocation = "az acr build `"
        assert normalized.index("deploymentstage=bootstrap") < normalized.index(
            build_invocation
        )
        assert f'--image "{repository}:$sourcecommit"' in normalized
        assert normalized.index(build_invocation) < normalized.index(
        ...
        assert normalized.count("az deployment group create") == 3
