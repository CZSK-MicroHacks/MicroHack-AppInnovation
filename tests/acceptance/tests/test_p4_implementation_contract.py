"""Cross-layer checks for the refrozen P4 Azure implementation."""
...
def test_dotnet_exports_all_signals_to_azure_monitor_without_losing_otlp(
    ...
    project = (
        repo_root / "dotnet/src/LegoCatalog.App/LegoCatalog.App.csproj"
    ).read_text(encoding="utf-8")
    program = (
        repo_root / "dotnet/src/LegoCatalog.App/Program.cs"
    ).read_text(encoding="utf-8")
...
def test_java_uses_the_locked_azure_monitor_autoconfigure_versions(
    ...
    application = (
        repo_root
        / "java/src/main/java/com/microsoft/microhack/catalog/CatalogApplication.java"
    ).read_text(encoding="utf-8")
    options = (
        repo_root
        / "java/src/main/java/com/microsoft/microhack/catalog/config/"
        "CatalogRuntimeOptions.java"
    ).read_text(encoding="utf-8")
