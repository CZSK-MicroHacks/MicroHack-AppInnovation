...
| Pipeline lead time — dispatch to live | | | `evidence/ch00-pain-<stack>.json` → `manualDeploySteps` and `manualDeployWindow`, recorded at the end of Challenge 0, step 3 | `evidence/cicd-report.json` → `.workflow.jobs.staging.startedAt` to `.traffic.promotion.observedAt`, printed by the Challenge 3, step 5 `jq` |
