...
| Your image assessment query fails outright with `SubAssessmentsDeprecated` and `az rest` exits non-zero | Defender for Cloud has deprecated the sub-assessments endpoint; the frozen request now answers HTTP 404. This is a failed query, not an empty one | Record the error as the response and set that signal's status to `unavailable`. Do not confuse this with the asynchronous-findings row below: an aborted query has no result to age into. Under `set -euo pipefail` an unguarded `RESPONSE=$(az rest ...)` kills the whole capture, so guard that one assignment. |
