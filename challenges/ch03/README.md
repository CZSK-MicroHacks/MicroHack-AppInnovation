...
| The staging smoke probes retry until they time out, and the URL answers from the migration-source VM but not from the runner | The Container Apps environment was deployed internal, so ingress is published only inside the virtual network and a GitHub-hosted runner cannot reach it at all. The facilitator sets this when the subscription blocks public IP creation — check with `az containerapp env list --query "[].{name:name,internal:properties.vnetConfiguration.internal}" -o table` | This is an environment mode, not a mistake you made. The probes have to originate from a peered host, so the run needs a self-hosted runner inside the virtual network. Confirm the mode with your facilitator before changing anything in the workflow |
